...
@SpringBootTest
class UserServiceTest @Autowired constructor(  // constructor 앞에 Autowired 붙이면 각각 프로퍼티 앞에 붙인것과 동일하다.
    private val userRepository: UserRepository,
    private val userService: UserService,
    private val userLoanHistoryRepository: UserLoanHistoryRepository,
) {

    // 각 테스트가 끝날때마다 디비를 비워줌 그래야 각 테스트간의 데이터가 겹치지 않음
    @AfterEach
    fun clean(){
    ...
    fun saveUserTest(){
        // given
        val request = UserCreateRequest("김슬기", null)
        ...
        assertThat(results[0].name).isEqualTo("김슬기")
